...
namespace SpaceBattle.Spaceship
{
    ...
    public class SpaceshipMovement : NetworkBehaviour
    {
        ...
        private void CalcAxis()
        {
            ...
            speedAxis = Input.GetAxis("Vertical"); //up&down. controls the speed
        ...
        {
            //int shouldApplyForce = (Mathf.Abs(vertAxis)>0.05f || (rb.velocity.magnitude < minSpeed)) ? 1 : 0;
            float forceCoffecient = 0;
            if (speedAxis > 0.05f || speedAxis < -0.05f)
            {
                forceCoffecient = speedAxis;
            }
